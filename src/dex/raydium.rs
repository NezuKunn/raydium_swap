use crate::utils::{raydium_h, raydium_p};
use eyre::Result;
...
pub struct RaydiumCliemt<S, PC> {
    pub payer: Arc<S>,
    pub program_client: Arc<PC>,
}
    ...
    RaydiumCliemt<S, PC>
{
    ...
    pub async fn swap(
        ...
        let ata_creation_bundle = raydium_h::get_or_create_ata_for_token_in_and_out_with_balance(
            &token_in,
            &token_out,
            self.payer.clone(),
        )
        ...
        let pool_info = raydium_p::get_pool_info(token_in.get_address(), token_out.get_address())?
